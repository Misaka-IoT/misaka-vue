  ...
  <ul class="WriteIndex">
    <li class="WriteItem" v-for="(i, j)  in item" :key="i.id" >
      <div style="height: 100px;">
        <h1 style="width: 80%;float: left ;user-select: none;height: 80%;" @click="ReadSwich(i.id)">{{ i.title }}</h1>
        <h2 style="left: 80%;top: -54%;position: relative;width: fit-content;height: 100%;">{{ i.star }}</h2>
        <button style="float: right;top:-54%" class="icon-btn standard no-color"  @click="OnStar(i.id)">
...
export default {
    ...
    methods: {
      ...
      IsIcon(id:string)
      {
        if(this.dzlb.length>0 && this.dzlb.findIndex((element) => element == id)>-1 )
        {
